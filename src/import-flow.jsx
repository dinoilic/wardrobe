...
export function WardrobeImportFlow({ onGarmentApproved, onModeledApproved }) {
  ...
  const setupNeeds = setupRequired ? [
    setup.missingConfig?.length ? setup.missingConfig.join(" and ") : setup.hasApiKey ? null : `your ${setup.providerLabel || "OpenAI"} API key to .env`,
    setup.hasModelReference ? null : <>a PNG reference photo of yourself at <code>{setup.modelReference || "data/model-reference.png"}</code></>,
  ].filter(Boolean) : [];
    ...
    <>
      ...
      <div className="import-popover-backdrop" data-open={open} onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
        <section className="import-popover" role="dialog" aria-modal="true" aria-labelledby="import-title">
          ...
          {!jobs.length ? setupRequired ? <div className="import-drop-target import-setup-warning"><WarningCircle size={30} /><h2>Setup required</h2><p>Add {setupNeeds.map((need, index) => <span key={index}>{index > 0 ? " and " : ""}{need}</span>)}, then restart the app.</p></div> : <div className="import-drop-target"><UploadSimple size={28} /><h2>{notice ? "Try another image" : "Choose or paste an image"}</h2><p>{notice?.detail || "We’ll isolate each clothing item, suggest its details, and hold everything for your approval."}</p><button className="import-button import-button--primary" disabled={!setup?.ready} onClick={() => { setNotice(null); inputRef.current?.click(); }}>Choose images</button></div> : (
            <>
